...
11. **Store.** Assemble the entry in the scratch directory, checksum every block, rename into the local store, add the manifest candidate, and hand the entry to the remote client (spooled or synchronous per policy).
