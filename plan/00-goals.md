...
## Design input: the extension surface must be easy to reason about

buildcache's flexibility goal is right: a user's rule for a new tool, a shipped rule for gcc, and a compiled-in handler are one surface, so nothing the project ships is out of a user's reach. Its implementation of that goal is its weakest part. A Lua wrapper is a set of hook functions (`get_relevant_arguments`, `get_build_files`, `preprocess_source`, ...), each hand-parsing argv, and an author has to hold the engine's call order in their head to know what each hook may assume. Every wrapper re-does the same flag walking. The result is hard to read, hard to reason about, and easy to get subtly wrong, which its issue tracker confirms (`plan/research/prior-art/buildcache.md`).

What api-cache keeps: one surface. Shipped rules and user rules are the same kind of file, a user can read, copy and override a shipped rule, and no tool knowledge lives only in Go.

What api-cache must do better: a rule has to be understandable by reading it, and `explain` has to be able to show why an invocation was handled the way it was in the rule's own terms. Busywork that every rule would otherwise repeat (response-file expansion, a tool's argument syntax, dependency-file parsing and rewriting, `/showIncludes` parsing, derived sibling outputs, stderr normalization) belongs in the engine as named behaviors, whatever shape the rule surface takes.

The shape of that surface is an open decision, worked through in `02-language.md` with worked examples and recorded in `12-decisions.md`. The candidates on the table:

- **A declarative table of meanings.** A rule lists argument patterns with a role each (input, output, dependency output, read path, written path, hash, ignore, bypass with a reason). Easy to read and to explain, and covers the common case of a compiler-shaped tool. Weakest when a rule needs computation.
- **api-cli's shape.** `<flag>` declarations plus passthrough `.rest`, `<vars>`, `<steps>` and templates, which is what the `cicc-cache` example in api-cli's README already sketches. Maximally flexible, already implemented, and reads as a pipeline rather than as a table. Weakest at showing, at a glance, what each argument means.
- **A hybrid.** The table for classification, api-cli's steps and templates for the parts that compute (a preprocess step, a program identity probe, a derived path), with the engine primitives named in both. The most likely answer, and the one whose readability has to be proven with real rule files for gcc, MSVC and rustc before it is chosen.
