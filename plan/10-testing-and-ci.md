...
- **Restore and filesystems**: per-method tests that run on every platform and report the verdict rather than skipping. On the Linux runner the store tests run against loop-mounted filesystems created in the job with `sudo`: ext4, XFS with `reflink=1` and with `reflink=0`, ZFS via `zfsutils-linux` with `compression=off`, `lz4` and `zstd`, btrfs, and overlayfs over ext4; NTFS and APFS run natively on their runners. Each filesystem asserts the functional probe's verdict, the byte-identity of a restored file, that writing to a cloned restore leaves the cache entry untouched, and the fallback-and-log path when the optimised operation is made to fail.
