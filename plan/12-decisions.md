...
**Decided by the user: binpazer**, with pooled codecs, sized reads, binpazer's per-block CRC (the only integrity the format has; ~40 µs on the miss path, verification only on a hit), and a raw-file escape for clone and link restore.
...
**Recommended: compress by default on every platform (zstd-1); restore by copy by default; clone automatic wherever the probe passes and the entry is uncompressed anyway (ZFS with dataset compression, `compress="never"`), opt-in where it would cost the compression; hard link always opt-in.** The 34x clone on APFS is real but a Mac is where storage is scarcest and CPUs fastest, so capacity wins the default. Filesystem-specific paths follow the four rules in `04-local-store.md` (functional probe, opt-in when the format changes, fallback-and-log, CI on that filesystem), for the set actually in use: ext3/4, XFS, ZFS, APFS, NTFS, overlayfs. Follow-up measurement that would change the APFS answer: APFS transparent per-file compression (decmpfs). It is written directly from user space with no second pass, the way `ditto --hfsCompression` does it: an empty data fork, a `com.apple.decmpfs` xattr with the `fpmc` header, the compressed payload inline for tiny files or in the resource-fork xattr as a chunk table of 64 KiB raw zlib streams for larger ones, then `chflags UF_COMPRESSED`. Go's stdlib zlib can produce it without cgo. `clonefile` preserves the xattrs and flag, and a write to the clone decompresses only the clone. Two caveats to measure on macos-latest before adopting: the decompression cost moves from the wrapper to the next reader of the object (a kernel zlib inflate, roughly half the speed of our zstd decode), so wrapper latency drops while total CPU per hit stays flat or rises; and the format is Apple-internal, stable for fifteen years but undocumented.
