...
- **Write:** assemble in `tmp/` (same filesystem), CRC each block (binpazer `has_crc`), rename into place. On NTFS rename over an open file fails (measured), so the store renames the old entry aside to a `.old-*` name and unlinks it after; a failure to unlink is a retry, never an error.
...
binpazer's own mechanism, used as designed: it has no whole-file checksum (its footer is a fixed 16 bytes of index offset and magic, and its header signature only catches line-ending mangling), so integrity is the optional per-block CRC-32C (`has_crc`) over each block's stored bytes, set on every block and checked on every block a read touches, before any decoder runs. A raw sibling file (clone or link mode) has its own CRC recorded in the directory block, since that file has no header. The cost: 23 µs per 512 KiB on x86 and 67 µs on the M1 to verify on a hit, and about 40 µs (~11% of pack) on the miss path, next to a compile that costs milliseconds; free on read beyond the verification itself. The checksum exists for the no-fsync rename path, a garbled remote transfer, and our own writer's bugs; it is not for bit rot, which only ZFS guards against among the filesystems in use (APFS checksums metadata only; ext4, XFS and NTFS checksum no file data by default). Never SHA-256 for integrity: 62x the cost on a machine without `sha_ni` for nothing, since integrity is not authenticity. An entry that fails its CRC is treated as a miss, deleted, counted (`corrupt_entry`), and logged once per build.

## Filesystems, and the rule for filesystem-specific code

The filesystems in scope are the ones actually in use: ext3 and ext4, XFS, ZFS (Linux and macOS), APFS, NTFS, and overlayfs inside Docker (where the store is normally a bind-mounted host directory, so the host filesystem applies). Every one of them runs the same portable path: write a temp sibling, CRC each block, rename, restore by copy, verify on read. That path is the default everywhere and is the only path that is not optional.
...
`restore="auto"` probes once per store (a throwaway file, the way go-s3-server probes atime) and records which methods the filesystem supports. The resolution rule:

- **Clone is automatic whenever it costs nothing**: the functional probe passes and the entry is stored uncompressed anyway, which is the case on a ZFS dataset with compression on (the advisory turns our compression off and ZFS compresses the cloned blocks itself), on a `compress="never"` store, and on APFS if the decmpfs follow-up lands. Clone is copy-on-write and safe to write to, so there is no correctness reason to withhold it.
- **Clone is opt-in (`restore="clone"`) when taking it would mean giving up our compression**, because that is a capacity trade the user makes, not the store. Choosing it forces `compress="never"`.
- **Copy is the default otherwise**, because it is the only method compatible with a compressed entry.
- **Hard link is always opt-in (`restore="link"`)** and needs the rule's `hard-link="safe"`: a linked restore shares an inode with the cache, and anything that writes the restored file in place corrupts the entry for everyone. ccache and buildcache both default it off; the plan does too. The reason the default is not clone on APFS, despite the 34x: a Mac is the machine where storage is the scarce resource (no expansion, expensive, fast CPUs), so the 1 ms per hit that decompression costs there (~4 s on a 4,000-file build) is worth 4.3x more entries in the same budget. A user who prefers speed over space sets `restore="clone"` and gets the measured 167 µs.
