...
Nothing in this design spends effort keeping an entry alive through something nobody does to a cache. A damaged, stale, mismatched or suspicious entry is deleted and regenerated by the next miss; a damaged store is deleted and regenerated by the next build. That is why there is no fsync, why a format-fingerprint mismatch purges the tree, why a CRC failure is a delete rather than a repair, and why go-s3-server's self-heal machinery is not lifted. The one thing the store must never do is serve wrong bytes; everything short of that is a miss.
