...
- A workflow's push trigger is scoped with `paths:` to exactly two files: the workflow file itself and `plan/research/<worker>/TRIGGER`. A worker starts a run by writing the current timestamp into its TRIGGER file and pushing; ordinary progress commits never start a run. No `concurrency:` block at all. Keep `workflow_dispatch`.
